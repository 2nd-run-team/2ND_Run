...
작업 브랜치는 **`<타입>/<작업>`** 형식이고, 타입은 **직군 기준 3종**입니다.
...
규칙:
- **작성자 이름은 넣지 않습니다.** GitHub 가 PR·브랜치 목록·blame 에 이미
  작성자를 표시합니다.
- 영소문자와 하이픈만 씁니다. 한글·공백·대문자는 OS 마다 다르게 동작합니다.
- 2~4 단어로 짧게. 자세한 설명은 PR 본문에 씁니다.
- 작업 브랜치는 `develop` 에서 땁니다. `main` 에서 따면 PR 을 `develop` 으로
  올릴 때 히스토리가 어긋납니다.
- 머지된 브랜치는 삭제합니다 (PR 머지 시 자동 삭제됨).

접두사를 직군에 맞춘 이유는 **브랜치 목록만 봐도 누가 뭘 하는지 보이게**
하기 위해서입니다. `art/` 와 `sound/` 는 LFS 락이 필요한 작업이고
`feature/` 는 대부분 필요 없어서, 락 충돌을 예상하기도 쉬워집니다.

버그 수정·설정·문서도 전부 `feature/` 를 씁니다. 브랜치 이름으로는 구분되지
않지만 **커밋 메시지 타입 8종(`feat` `fix` `refactor` `art` `level` `docs`
`chore` `ci`)은 그대로 쓰므로** 히스토리에서는 구분됩니다 (3번 참고).
